...
const handleDuplicateFieldsDB = (err) => {
  ...
  const msg = `Duplicate Fields: ${value}. Use another!`;
  return new AppError(msg, 400);
};
...
const handleJWTExpiredError = (err) => new AppError("TOKEN HAS EXPiRED", 401);
...
const sendErrorProd = (err, res) => {
  if (err.isOperational) {
    ...
    console.log("ERROR WOOP WOOP", err);
    res.status(500).json({
      status: "error",
      message: "Very bad things, dude!",
    });
  }
};
